...
function buildProPayload() {
  ...
  const active = staff.map((emp, idx) => ({
    ...
    leaveDates: leaves
      .filter((l) => Number(l.staffId) === Number(emp.id))
      .map((l) => l.date),
  ...
  return {
    employees: active,
    shiftSettings: shifts.map((s) => ({
      id: Number(s.id),
      code: String(s.id),
      name: s.name || s.id,
      start: s.start,
      end: s.end === "00:00" && s.id === "PM" ? "24:00" : s.end,
...
function applyGeneratedSchedule(data) {
  ...
  data.dates.forEach((date) => {
    schedule[mk][date] = {};
    shifts.forEach((s) => (schedule[mk][date][s.id] = []));
  });
...
async function generateSchedulePro() {
  ...
  try {
    ...
    console.error(err);
    showToast(T.backend_connection_failed);
  }
}
function schedulesForBackend() {
  ...
  dates.forEach((date, day) => {
    shifts.forEach((s, shiftIndex) => {
      (getDaySchedule(date)[s.id] || []).forEach((id) => {
...
async function autoFixSchedulePro() {
  ...
  try {
    ...
    showToast(
      T.auto_fix_done(data.unmet?.length || 0, data.violations?.length || 0),
    );
...
async function login() {
  ...
  try {
    ...
    showToast(
      err.name === "AbortError" ? T.login_timeout : T.login_connection_failed,
    );
  } finally {
    btn.disabled = false;
...
function refreshTopAuthUI() {
  ...
  document.getElementById("login-top-btn").style.display = loggedIn
    ? "none"
    : "inline-flex";
  document.getElementById("menu-top-btn").style.display = loggedIn
    ? "inline-flex"
    : "none";
  ...
  if (status)
    status.textContent = loggedIn
      ? T.logged_in(currentCompanyCode)
      : T.login_status;
}
let authMode = "login";
...
function setAuthMode(mode) {
  ...
  document.getElementById("auth-sub").textContent =
    mode === "login" ? T.auth_sub_login : T.auth_sub_register;

  document.getElementById("auth-submit-btn").textContent =
    mode === "login" ? T.login : T.create_continue;

  document.getElementById("auth-switch-text").innerHTML =
    mode === "login" ? T.auth_switch_register_html : T.auth_switch_login_html;
}
