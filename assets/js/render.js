...
{const el=document.getElementById(id);if(!el)return;el.innerHTML='';
  T.days.forEach(d=>{const h=document.createElement('div');
    h.className='dow';h.textContent=d;el.appendChild(h)});
    monthDates(false).forEach(cell=>{const div=document.createElement('div');
      div.className='day-card'+(!cell?' muted':'');if(!cell){el.appendChild(div);
        return}const date=ymd(cell.date);const ds=getDaySchedule(date);let badges='';
        if(isHoliday(date))badges+=`<span class="holiday-chip">${T.holiday}</span>`;
        const lf=leavesForDate(date);if(lf.length)badges+=`<span class="leave-chip">${T.leave} ${lf.length}</span>`;div.innerHTML=`<div class="day-head"><div class="day-num">${cell.date.getDate()}</div><div>${badges}</div></div>`;shifts.forEach(s=>{const assigned=ds[s.code]||[];
...
row.className = 'shift-row ' + ((missing || holidayConflict || leaveConflict) ? 'bad' : '');
